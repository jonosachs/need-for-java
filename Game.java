...
public class Game
{
    ...
    */
    public void checkCellForObstacles(int x, int y)
    {
        ...
        {
            case "F": //Fuel
                if((player.getVehicle().getCurrentFuel() + 10) > player.getVehicle().getMaxFuel()) //check if fuel will be > max
                    player.getVehicle().setCurrentFuel(player.getVehicle().getMaxFuel()); //if yes set to max fuel instead
                else
    ...
    */
    ...
    {    
        ...
        if (difficulty == 1)
        {
            highway.setLaneLength(random.genRandomNum(10, 15));
            fuelHandicap = 1;
            highway.setMaxObstacles(12);
        }
        if (difficulty == 2)
        {
            highway.setLaneLength(random.genRandomNum(15, 30));
            fuelHandicap = 0.8;
            highway.setMaxObstacles(24);
        }
        if (difficulty == 3)
        {
            highway.setLaneLength(random.genRandomNum(30, 50));
            fuelHandicap = 0.5;
    ...
    */
    ...
    /**
    *Creates the highway via the Highway class and sets a random player starting lane
    */
    public void createHighway()
    {
        ...
        Random random = new Random();
        int num = random.genRandomNum(0, 2);
    ...
    *@return                Returns vehicle details formatted as a StringConfiguration is still incorrect. Do you want to edit it again?
    */
